...
from UnknownFW.load_data import load_pt_raw_data_twibot22_100w
from UnknownFW.load_data import subgraph_hetero_to_homo
from UnknownFW.Graph_Split import Cluster_Split_Shuffle
...
if __name__ == '__main__':
    ...
    #! 选择使用哪个数据集
    parser.add_argument('--dataset', type=str, default='twibot_22_100w_shuffle')
    parser.add_argument('--dataset_name', type=str, default='twibot_22_100w_shuffle')
    parser.add_argument('--subgraph_num', type=int, default=800, help='子图数量')
    ...
    parser.add_argument('--generation_epochs', type=int, default=200)
    ...
    args_formated = vars(args)
    print(json.dumps(args_formated, indent=4))
    
    #! 读取原始图
    load_not_split = True
    Original_Graph = load_pt_raw_data_twibot22_100w()
    print('Raw split!!!!!!!!')
    ...
    if load_not_split:
        ...
        SubGraph_List = Cluster_Split_Shuffle(Original_Graph, 800)
    ...
    for edge_type_index in range(2):
        #! 创建日志记录器
        main_logger = mp_logger('twibot_22_way_2_subgraph_augmented_type_'+str(edge_type_index+1))
        main_logger.info('Augmenting Type '+str(edge_type_index+1)+' Homo SubGraphList......')
        
        #! 分离出每个异质子图的同质边(同质图)
        subgraph_hetero_to_homo_list = subgraph_hetero_to_homo(SubGraph_List, args.dataset, 2)
        ...
        #! 保存增强后的结果
        torch.save(AugmentedGraph_List, '/data3/xupin/0_UNName/data/way_2/'+ args.dataset_name+
