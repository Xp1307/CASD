...
class BotRGCN_Trainer(object):
    def __init__(self, args, hetero_subgraph, raw_subgraph):
        ...
        #! 创建可训练的权重参数
        self.weight1 = torch.nn.Parameter(torch.tensor(1, dtype=torch.float32, requires_grad=True).to('cuda:'+str(args.device_id)))
        self.weight2 = torch.nn.Parameter(torch.tensor(1, dtype=torch.float32, requires_grad=True).to('cuda:'+str(args.device_id)))
    ...
    def concat_train(self, logger=None):
        ...
        # class_weights = torch.tensor([0.5, 2.0], dtype=torch.float).to('cuda:{}'.format(self.args.device_id))
        ...
        # class_weights = torch.tensor([each/class_sample_count.sum() for each in reversed(class_sample_count)], 
        #                             dtype=torch.float)
        # class_weights = torch.tensor([1+class_weights[0]*(class_weights[0]-class_weights[1]),
        #                               1+class_weights[1]*(class_weights[1]-class_weights[0])], 
        #                             dtype=torch.float).to('cuda:{}'.format(self.args.device_id))
        # loss_fn = torch.nn.CrossEntropyLoss(weight=class_weights)
        ...
        for epoch in range(args.epochs):
            ...
            # 设置权重
            # total_weight = self.weight1 + self.weight2
            # normalized_weight1 = self.weight1 / total_weight
            # normalized_weight2 = self.weight2 / total_weight
            # train_out = self.concat_classifier(torch.cat((
            #     torch.mul(out_embedding[self.data.train_mask], normalized_weight1),
            #     torch.mul(raw_embedding[self.raw_data.train_mask], normalized_weight2)
            # ), dim=1))
            train_out = self.concat_classifier(torch.cat((
...
if __name__ == '__main__':
    ...
    #! 数据集名字
    parser.add_argument('--dataset', type=str, default='twibot_22_100w', help='数据集名')
    parser.add_argument('--dataset_name', type=str, default='twibot_22_100w', help='数据集名')
    ...
    hetero_graph_list = torch.load('/data3/xupin/0_UNName/data/way_2/twibot22_100w/'+dataset_name+'_subgraphlist_hetero_to_type_1_node_hetero.pt',
                                   map_location=device)
    raw_graph_list = torch.load('/data3/xupin/0_UNName/data/way_2/twibot22_100w/'+dataset_name+'_subgraphlist.pt',
                                map_location=device)
