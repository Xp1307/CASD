...
class HGT(nn.Module):
    def __init__(self, args, metadata):
        ...
        self.pe = args.pe
        self.pf = args.pf
        input_dimension = args.input_dim            ## 788
        embedding_dimension = args.hidden_dim       ## 32
        
        self.linear_relu_tweet=nn.Sequential(
            nn.Linear(768, int(embedding_dimension*1/2)),
            nn.LeakyReLU()
        )
        self.linear_relu_num_prop=nn.Sequential(
            nn.Linear(10, int(embedding_dimension/4)),
            nn.LeakyReLU()
        )
    ...
    def forward(self, data):
        x = data['user'].x
        t = self.linear_relu_tweet(x[:, -768:].to(torch.float32))
        n = self.linear_relu_num_prop(x[:, [4,6,7,8,10,11,12,13,14,15]].to(torch.float32))
        b = self.linear_relu_cat_prop(x[:, [1,2,3,5,9,16,17,18,19,20]].to(torch.float32))
